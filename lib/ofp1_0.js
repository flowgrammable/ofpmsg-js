...
function Hello(data) {
  this.VERSION = new uint.UInt({ bytes: 1, value: VERSION });
  this.Type    = new uint.UInt({ bytes: 1, value: Hello.Type });
  dt.Data.call(this, data);
}
util.makePayload(Hello, 0, dt.Data);

Hello.prototype.bytes = function(){
  return new uint.UInt({ bytes: 2, value: dt.Data.prototype.bytes.call(this)});
};
...
function FeatureReq(){
  this.VERSION = new uint.UInt({ bytes: 1, value: VERSION });
  this.Type    = new uint.UInt({ bytes: 1, value: FeatureReq.Type });
}
util.makePayload(FeatureReq, 5, dt.Data);

FeatureReq.prototype.fromView = function(view){
 if(view.available() < this.bytes()){
  throw util.Available('FeatureReq');
 }
}; 

FeatureReq.prototype.bytes = function(){
  return new uint.UInt({bytes:2, value:0});
};
...
FeatureRes.prototype.bytes = function(){
  return 24 + dt.Data.prototype.bytes.call(this);
};
