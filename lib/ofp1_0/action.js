...
function SetVLANPCP(vlanpcp){
  this.Type    = uint.mk(1, SetVLANPCP.Type);
  this.vlanpcp = vlanpcp || null;
}
util.makePayload(SetVLANPCP, 2);

SetVLANPCP.prototype.bytes = function(){
  return uint.mk(2, 4);
};

SetVLANPCP.prototype.fromView = function(view){
  if(view.available() < this.bytes()){
   throw util.Available('SetVLANPCP');
  }
 this.vlanpcp = view.readUInt8();
 util.pad(view, 3);
 return this;
}; 

SetVLANPCP.prototype.toView = function(view){
  if(view.available() < this.bytes()){
    throw util.Available('SetVLANVid');
  }
  view.writeUInt8(this.vlanpcp);
  util.pad(view, 3);
  return this;
};

exports.SetVLANPCP = function(args) {
  var vlanpcp = null;
  if(args){
    vlanpcp = args.vlanpcp || null;
  }
  return new Action({ payload: new SetVLANPCP(vlanpcp) });
};

function StripVLAN(){
  this.Type    = uint.mk(1, StripVLAN.Type);
}
util.makePayload(StripVLAN, 3);

StripVLAN.prototype.bytes = function(){
  return uint.mk(2, 0);
};

StripVLAN.prototype.fromView = function(view){
 return this;
}; 

StripVLAN.prototype.toView = function(view){
  return this;
};

exports.StripVLAN = function() {
  return new Action({ payload: new StripVLAN() });
};
