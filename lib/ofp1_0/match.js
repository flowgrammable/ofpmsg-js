...
function Match(args) {
  ...
  this.internal = {
    phy_port: new uint.UInt({ bytes: 2 })
  };
  this.ethernet = {
    src: new ethernet.MAC(),
    dst: new ethernet.MAC(),
    vlan: new uint.UInt({ bytes: 2 }),
    pcp:  new uint.UInt({ bytes: 1 }),
    type: new uint.UInt({ bytes: 2 })
  };
  this.ipv4 = {
    tos: new uint.UInt({ bytes: 1 }),
    proto: new uint.UInt({ bytes: 2 }),
    src: new ipv4.Address(),
    dst: new ipv4.Address() 
  };
  this.transport = {
    src: new uint.UInt({ bytes: 2 }),
    dst: new uint.UInt({ bytes: 2 })
  };
...
Match.prototype.toView = function(view) {
  ...
  this.ethernet.src.toView(view);
  this.ethernet.dst.toView(view);
  ...
  util.pad(view, 2);
  this.ipv4.src.toView(view);
  this.ipv4.dst.toView(view);
