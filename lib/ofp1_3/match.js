...
Match.prototype.fromView = function(view){
  ...
  this.oxmView = view.constrain(this.length.value() - 4);
  // TODO: rework
  while(this.oxmView.available > 8){
    var tmp = new OXM().fromView(this.oxmView);
    this.oxms.push(tmp);
  }
  // Padding
...
OXM.prototype.fromView = function(view){
  ...
  if(view.available() < this.length.value()){
    throw util.Available('OXM Payload', this.length.value());
  }
